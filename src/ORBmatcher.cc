...
namespace ORB_SLAM3
{
                                       ...
                                       vector<MapPoint*> &vpMatched, int th, float ratioHamming)
    {
        ...
        for(int iMP=0, iendMP=vpPoints.size(); iMP<iendMP; iMP++)
        {
            ...
            std::vector<cv::KeyPoint> keypoints1; // 当前图像中的关键点
            std::vector<cv::Mat> descriptors1; // 当前图像的描述符

            // 获取当前关键帧的关键点和描述符
            keypoints1 = pKF->mvKeysUn; // 当前关键帧中的所有关键点
            descriptors1 = pKF->mDescriptors; // 当前关键帧的描述符

            // 使用 LightGlue 匹配当前 MapPoint 的描述符和关键帧的描述符
            lightglue.matchDescriptors(keypoints1, descriptors1, pMP->GetDescriptor(), matches);

            // 选择最好的匹配
            int bestIdx = -1;
            float bestDistance = std::numeric_limits<float>::max(); // 最小距离初始化为最大值

            for (const auto& match : matches) {
                // 检查是否找到最好的匹配
                if (match.distance < bestDistance) {
                    bestDistance = match.distance;
                    bestIdx = match.trainIdx; // 保存最佳索引
                }
            }

            // 如果找到合适的匹配
            if (bestDistance <= TH_LOW) { // TH_LOW 是根据需要定义的距离阈值
                vpMatched[bestIdx] = pMP; // 将匹配的 MapPoint 记录下来
                nmatches++; // 匹配计数器增加
            }
        }
