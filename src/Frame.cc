...
{
    // 初始化用于存储右图像中对应点横坐标和深度信息的向量，初始值设为 -1.0f，表示未匹配或无效值
    mvuRight = vector<float>(N, -1.0f);
    mvDepth = vector<float>(N, -1.0f);

    int imgWidth = mnMaxX - mnMinX;  // 图像的宽度，通过图像最大横坐标减去最小横坐标得到
    int imgHeight = mnMaxY - mnMinY; // 图像的高度，通过图像最大纵坐标减去最小纵坐标得到

    // 创建两个表示图像尺寸的张量，数据类型为torch::kFloat32，用于后续LightGlue匹配器可能需要的尺寸相关输入
    ...
    // 将OpenCV的Mat描述符转换为torch::Tensor，用于后续匹配操作
    // 首先声明用于存储左、右图像描述符的torch::Tensor变量
    torch::Tensor leftDesc, rightDesc;

    // 检查左图像描述符是否有效（非空且行列数合理），若无效则打印错误信息并直接返回，避免后续出现非法操作
    if (mDescriptors.empty() || mDescriptors.rows == 0 || mDescriptors.cols == 0)
    {
        std::cerr << "左图像描述符无效，无法进行立体匹配操作！" << std::endl;
        return;
    }
    // 将左图像描述符转换为torch::Tensor，传入相应的指针、行列数以及数据类型信息
    leftDesc = torch::from_blob(const_cast<float*>(mDescriptors.ptr<float>()), {static_cast<long>(mDescriptors.rows), static_cast<long>(mDescriptors.cols)}, torch::kFloat);

    // 检查右图像描述符是否有效（非空且行列数合理），若无效则打印错误信息并直接返回
    if (mDescriptorsRight.empty() || mDescriptorsRight.rows == 0 || mDescriptorsRight.cols == 0)
    {
        std::cerr << "右图像描述符无效，无法进行立体匹配操作！" << std::endl;
        return;
    }
    // 将右图像描述符转换为torch::Tensor
    rightDesc = torch::from_blob(const_cast<float*>(mDescriptorsRight.ptr<float>()), {static_cast<long>(mDescriptorsRight.rows), static_cast<long>(mDescriptorsRight.cols)}, torch::kFloat);

    std::cout << "Frame中的匹配" << std::endl;

    // 执行特征匹配前，先检查左、右图像关键点向量是否为空，若为空则打印错误信息并返回，因为没有关键点无法进行匹配
    if (mvKeys.empty() || mvKeysRight.empty())
    {
        std::cerr << "左或右图像关键点向量为空，无法执行特征匹配！" << std::endl;
        return;
    }
    // 执行特征匹配
    std::vector<cv::DMatch> matches = lg.matchDescriptors(mvKeys, leftDesc, mvKeysRight, rightDesc, size1, size2);

    const float minZ = mb;
    const float minD = 0;
    const float maxD = mbf / minZ;

    vector<pair<float, int>> vDispIdx;
    vDispIdx.reserve(matches.size());

    for (const auto& match : matches)
    {
        const int iL = match.queryIdx;
        const int iR = match.trainIdx;

        // 检查匹配的关键点索引是否超出对应关键点向量的范围，若超出则跳过该匹配，避免越界访问
        if (iL >= N || iR >= (int)mvKeysRight.size())
            continue;

        const float uL = mvKeys[iL].pt.x;
        const float uR = mvKeysRight[iR].pt.x;
        ...
        if (disparity >= minD && disparity < maxD)
        {
            ...
            float distance = 1.0f - match.distance;
            vDispIdx.push_back(make_pair(distance, iL));
        }
    }
    ...
    sort(vDispIdx.begin(), vDispIdx.end());
    const float median = vDispIdx[vDispIdx.size() / 2].first;
    const float thDist = 1.5f * median;

    for (int i = vDispIdx.size() - 1; i >= 0; i--)
    {
        if (vDispIdx[i].first < thDist)
            break;
        else
        {
            mvuRight[vDispIdx[i].second] = -1;
            mvDepth[vDispIdx[i].second] = -1;
        }
    }
}
