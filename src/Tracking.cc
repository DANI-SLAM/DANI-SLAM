...
bool Tracking::ParseORBParamFile(cv::FileStorage &fSettings)
{
    ...
    if(!node.empty() && node.isReal())
    {
        fIniThFAST = static_cast<float>(node.operator double());
    ...
    if(!node.empty() && node.isReal())
    {
        fMinThFAST = static_cast<float>(node.operator double());
...
bool Tracking::TrackReferenceKeyFrame()//1.刚进行了重定位，2.恒速模型为空或恒速跟踪失败
{
...
bool Tracking::TrackLocalMap()
{
    ...
    {
        std::cout << "短跟踪" << mCurrentFrame.mnId << " " << mnLastRelocFrameId+mMaxFrames << " " << mnMatchesInliers << std::endl;
        return false;
    }
    ...
    {
        std::cout << "RECENTLY_LOST" << std::endl;
        return true;
    }
    ...
    {
        // std::cout << "others,有效匹配数" << mnMatchesInliers << std::endl;
...
float Tracking::CalculateDynamicObjectRatio(const std::vector<cv::Rect2i>& dynamicAreas, const Frame& F)
{
    float dynamicArea = 0.0;
    for (const auto& area : dynamicAreas)
    {
        dynamicArea += area.area(); 
    }
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(!mpLocalMapper->SetNotStop(true))
        return;
    // 计算当前帧的动态物体比例（通过YOLO检测结果）
    float dynamicRatio = CalculateDynamicObjectRatio(mpDetector->mvDynamicArea, mCurrentFrame); 
    if (dynamicRatio > 0.8) {
        return; 
    }
