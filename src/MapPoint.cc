...
{
    // 从所有观察到该地图点的关键帧提取 SuperPoint 描述符
    ...
    // 使用平均值作为代表描述符
    cv::Mat meanDescriptor = cv::Mat::zeros(1, vDescriptors[0].cols, CV_32F);
    for (const auto& desc : vDescriptors)
    {
        meanDescriptor += desc;
    }
    meanDescriptor /= static_cast<float>(vDescriptors.size());

    {
        unique_lock<mutex> lock(mMutexFeatures);
        mDescriptor = meanDescriptor.clone();
    }
}
