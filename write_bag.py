...
def get_camera_info():
    H, W, fx, fy, cx, cy = 480 , 640, 520.9, 521.0, 325.1, 249.7
    ...
    camera_info.D = [0.231222, -0.784899, -0.003257, -0.000105, 0.917205]
...
if __name__=="__main__":
    ...
    #        input_folder, 'results', '*.jpg')), key=lambda x: float(os.path.basename(x)[:-4]))
    color_paths = sorted(glob.glob(os.path.join(input_folder, 'rgb', '*.png')),
                     key=lambda x: float(re.search(r'\d+', os.path.basename(x)).group()))
    #depth_paths = sorted(glob.glob(os.path.join(
    #        input_folder, 'results', '*.png')), key=lambda x: float(os.path.basename(x)[:-4]))
    depth_paths = sorted(glob.glob(os.path.join(input_folder, 'depth', '*.png')),
                     key=lambda x: float(re.search(r'\d+', os.path.basename(x)).group()))
    ...
    n_img = min(len(color_paths), len(depth_paths))
